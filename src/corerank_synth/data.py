...
class SyntheticConfig:
    scenario: str = "complementary"  # complementary | redundant | biased
    seed: int = 0
    n_train: int = 5000
    n_val: int = 1000
    n_test: int = 2000
    ...
    def __post_init__(self) -> None:
        valid_scenarios = {"complementary", "redundant", "biased"}
...
def _make_params(cfg: SyntheticConfig) -> SyntheticParams:
    ...
    return SyntheticParams(fp, A, B, bias_vec, beta_y, cfg.noise_std)
...
def _generate_split(n: int, cfg: SyntheticConfig, params: SyntheticParams, split: str, rng: np.random.Generator) -> SyntheticSplit:
    z = rng.normal(0, 1, size=(n, cfg.z_dim)).astype(np.float32)
    ...
    for mm in range(cfg.n_modalities):
        ...
        x = (core_private + biased + noise).astype(np.float32)
