...
def permutation_invariant_structure_metrics(
    incidence_hat: np.ndarray,
    task_hat: np.ndarray,
    incidence_true: np.ndarray,
    task_true: np.ndarray,
    threshold: float = 0.5,
) -> Dict[str, float]:
    """Evaluate B and r after the factor permutation favored by joint structure.

    Latent coordinates are identifiable only up to permutation and componentwise
    transforms. Direct column-wise mask F1 is therefore a diagnostic, whereas
    this permutation-aligned score is the theorem-compatible structure metric.
    """
    pred_b = np.asarray(incidence_hat) >= threshold
    true_b = np.asarray(incidence_true) > 0
    pred_r = np.asarray(task_hat) >= threshold
    true_r = np.asarray(task_true) > 0
    if pred_b.shape != true_b.shape or pred_r.shape != true_r.shape:
        return {
            "incidence_f1_perm": float("nan"),
            "task_f1_perm": float("nan"),
            "structure_column_accuracy": float("nan"),
        }

    k = pred_b.shape[1]
    pred_signatures = np.concatenate([pred_b.astype(float), pred_r[None, :].astype(float)], axis=0)
    true_signatures = np.concatenate([true_b.astype(float), true_r[None, :].astype(float)], axis=0)
    agreement = np.empty((k, k), dtype=np.float64)
    for i in range(k):
        for j in range(k):
            agreement[i, j] = np.mean(pred_signatures[:, i] == true_signatures[:, j])
    try:
        from scipy.optimize import linear_sum_assignment

        pred_cols, true_cols = linear_sum_assignment(-agreement)
    except Exception:
        pred_cols = np.arange(k)
        true_cols = np.argmax(agreement, axis=1)

    aligned_b = np.zeros_like(pred_b)
    aligned_r = np.zeros_like(pred_r)
    for pred_col, true_col in zip(pred_cols, true_cols):
        aligned_b[:, true_col] = pred_b[:, pred_col]
        aligned_r[true_col] = pred_r[pred_col]
    return {
        "incidence_f1_perm": _binary_f1(aligned_b, true_b),
        "task_f1_perm": _binary_f1(aligned_r, true_r),
        "structure_column_accuracy": float(agreement[pred_cols, true_cols].mean()),
    }
