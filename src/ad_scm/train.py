...
class TrainConfig:
    ...
    counterfactual_demo_weight: float = 0.0
...
class ADCausalCorrectionNet(nn.Module):
    ...
    The embedding-conditioned variant keeps a learned graph skeleton and uses
    the model's continuous latent S embedding only to generate edge weights
    W(S). It does not introduce a clean label or a categorical state target.
    """
    ...
    def __init__(
        ...
        embedding_conditioned_graph: bool = False,
        ...
        self.embedding_conditioned_graph = embedding_conditioned_graph
        ...
        elif embedding_conditioned_graph:
            self.skeleton_logits = nn.Parameter(-0.20 + 0.03 * torch.randn(k, k))
            self.edge_weight_net = make_mlp(k, k * k, hidden_dim, layers)
        else:
    ...
    def skeleton(self) -> torch.Tensor:
        if self.embedding_conditioned_graph:
            return torch.sigmoid(self.skeleton_logits) * self.lower_mask
        if self.conditioned_graph:
            return (torch.sigmoid(self.graph_logits).mean(dim=0) * self.lower_mask)
        return (self.graph_raw.abs() > 0).float() * self.lower_mask

    def adjacency_from_embedding(self, s_embedding: torch.Tensor) -> torch.Tensor:
        if not self.embedding_conditioned_graph:
            raise ValueError("adjacency_from_embedding requires embedding_conditioned_graph=True")
        edge_weight = torch.tanh(self.edge_weight_net(s_embedding)).reshape(-1, self.k, self.k)
        return edge_weight * self.skeleton()[None, :, :]
    ...
    def adjacency_by_class(self) -> torch.Tensor:
        ...
        if self.embedding_conditioned_graph:
            zero_s = torch.zeros(1, self.k, device=self.lower_mask.device)
            return self.adjacency_from_embedding(zero_s)
        return (self.graph_raw * self.lower_mask)[None, :, :]

    def adjacency(
        self,
        setting_probs: torch.Tensor | None = None,
        s_embedding: torch.Tensor | None = None,
    ) -> torch.Tensor:
        if self.embedding_conditioned_graph:
            if s_embedding is None:
                s_embedding = torch.zeros(1, self.k, device=self.lower_mask.device)
            return self.adjacency_from_embedding(s_embedding)
    ...
    def state_from_z(
        ...
        a = self.adjacency(setting_probs, s_embedding)
        if a.dim() == 2:
    ...
    def forward(
        ...
        graph_s = proto if self.embedding_conditioned_graph else None
        s = self.state_from_z(z, setting_probs, graph_s)
        ...
        return {
            ...
            "recon": recons,
            "adjacency": self.adjacency(s_embedding=graph_s) if self.embedding_conditioned_graph else self.adjacency(setting_probs),
        }

    def graph_l1(self, adjacency: torch.Tensor | None = None) -> torch.Tensor:
        if self.embedding_conditioned_graph:
            value = adjacency.abs().mean() if adjacency is not None else torch.tensor(0.0, device=self.lower_mask.device)
            group = self.skeleton().mean()
            return group + value
    ...
    def dag_penalty(self) -> torch.Tensor:
        if self.embedding_conditioned_graph:
            b = self.skeleton()
            return torch.trace(torch.matrix_exp(b * b)) - b.shape[0]
        vals = []
...
def _warmup_for_xs(warmup: WarmupStateNet, xs: List[torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
    warmup.eval()
    with torch.no_grad():
        out = warmup(xs)
...
def _warmup_for_batch(
    ...
    return _warmup_for_xs(warmup, _select_xs(batch["x"], active_modalities))
...
def _ad_bc_loss(
    ...
    cf_demo = torch.tensor(0.0, device=batch["y"].device)
    active = _active_modalities(active_modalities, len(batch["x"]))
    if cfg.counterfactual_demo_weight > 0 and 3 in active and xs_full[0].shape[0] > 1:
        demo_pos = active.index(3)
        perm = torch.randperm(xs_full[demo_pos].shape[0], device=xs_full[demo_pos].device)
        xs_cf = [x.clone() for x in xs_full]
        xs_cf[demo_pos] = xs_cf[demo_pos][perm]
        proto_cf, setting_probs_cf = _warmup_for_xs(warmup, xs_cf)
        if model.conditioned_graph:
            setting_probs_cf = _mix_setting_probs(setting_probs_cf, batch["y"], cfg.graph_label_mix, model.n_classes)
        else:
            setting_probs_cf = None  # type: ignore[assignment]
        out_cf = model(xs_cf, proto_cf, setting_probs=setting_probs_cf, sample=False)
        cf_demo = F.mse_loss(out["logits"], out_cf["logits"])  # type: ignore[arg-type]
    return (
        ...
        + cfg.beta_z * kl_z
        + cfg.beta_u * kl_u
        + cfg.graph_l1_weight * model.graph_l1(out["adjacency"] if isinstance(out.get("adjacency"), torch.Tensor) else None)
        ...
        + cfg.counterfactual_demo_weight * cf_demo
...
def train_ad_bc_mcsgn(
    ...
    model = ADCausalCorrectionNet(
        ...
        embedding_conditioned_graph=embedding_conditioned_graph,
...
def _ad_bc_structure_metrics(
    ...
    graph_override: np.ndarray | None = None,
) -> Dict[str, float]:
    metrics: Dict[str, float] = {}
    avg_graph = graph_override if graph_override is not None else model.adjacency().detach().cpu().numpy()
    ...
    if model.embedding_conditioned_graph:
        graphs = avg_graph[None, :, :]
    else:
...
def _embedding_graph_mean(
    ...
    if not model.embedding_conditioned_graph:
        return None
    model.eval()
    warmup.eval()
    total = torch.zeros(model.k, model.k, device=cfg.device)
    count = 0
    for batch0 in _loader(split, cfg, shuffle=False):
        batch = _to_device(batch0, cfg.device)
        proto = _proto_for_batch(warmup, batch, active_modalities)
        a = model.adjacency_from_embedding(proto)
        total = total + a.sum(dim=0)
        count += a.shape[0]
    return (total / max(count, 1)).detach().cpu().numpy()
...
def train_and_evaluate_ad(
    ...
    baseline_specs = {
        ...
        "concat_no_demo": lambda: ConcatClassifier(scfg.x_dim, [0, 1, 2], tcfg.hidden_dim, tcfg.layers),
        ...
        "late_fusion_no_demo": lambda: LateFusionClassifier(scfg.x_dim, 3, tcfg.hidden_dim, tcfg.layers),
    }
    ...
    if (
        "warmup" in methods
        or "ad_bc_mcsgn" in methods
        or "ad_bc_mcsgn_scond" in methods
        or "ad_bc_mcsgn_s_embed" in methods
        or "mlcsl" in methods
    ):
    ...
    if "mlcsl" in methods and warmup is not None:
        model = train_ad_bc_mcsgn(
            splits["train"],
            splits["val"],
            scfg,
        ...
        if checkpoint_dir:
            torch.save(model.state_dict(), os.path.join(checkpoint_dir, "mlcsl.pt"))
        outputs = {split_name: evaluate_ad_bc(model, warmup, split, tcfg) for split_name, split in splits.items()}
        bc_rows = _evaluate_rows("mlcsl", outputs)
        if params is not None:
            structure_metrics = _ad_bc_structure_metrics(model, params, tcfg, params.modality_mask)
            for row in bc_rows:
                row.update(structure_metrics)
    ...
    if "ad_bc_mcsgn_s_embed" in methods and warmup is not None:
        model = train_ad_bc_mcsgn(
            splits["train"],
            splits["val"],
            scfg,
            tcfg,
            warmup,
            name="ad_bc_mcsgn_s_embed",
            embedding_conditioned_graph=True,
            decoder_uses_state=False,
        )
        if checkpoint_dir:
            torch.save(model.state_dict(), os.path.join(checkpoint_dir, "ad_bc_mcsgn_s_embed.pt"))
        outputs = {split_name: evaluate_ad_bc(model, warmup, split, tcfg) for split_name, split in splits.items()}
        bc_rows = _evaluate_rows("ad_bc_mcsgn_s_embed", outputs)
        if params is not None:
            graph_mean = _embedding_graph_mean(model, warmup, splits["train"], tcfg)
            structure_metrics = _ad_bc_structure_metrics(model, params, tcfg, params.modality_mask, graph_override=graph_mean)
            for row in bc_rows:
                row.update(structure_metrics)
        rows.extend(bc_rows)
    ...
    if "warmup_no_demo" in methods or "ad_bc_mcsgn_no_demo" in methods or "mlcsl_no_demo" in methods:
    ...
    if "mlcsl_no_demo" in methods and warmup_no_demo is not None:
        model = train_ad_bc_mcsgn(
            splits["train"],
            splits["val"],
            scfg,
        ...
        if checkpoint_dir:
            torch.save(model.state_dict(), os.path.join(checkpoint_dir, "mlcsl_no_demo.pt"))
        outputs = {
            split_name: evaluate_ad_bc(model, warmup_no_demo, split, tcfg, active_modalities=active_no_demo)
            for split_name, split in splits.items()
        }
        bc_rows = _evaluate_rows("mlcsl_no_demo", outputs)
        if params is not None:
            true_modality_mask = params.modality_mask[list(active_no_demo)]
            structure_metrics = _ad_bc_structure_metrics(model, params, tcfg, true_modality_mask)
            for row in bc_rows:
