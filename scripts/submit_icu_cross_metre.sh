...
for model in spmnet transformer; do
  job=$(sbatch --parsable \
    ...
    --job-name="metre_${model:0:3}_5seed" \
    --output="$RUN_ROOT/logs/${model}_5seed_%j.out" \
    --error="$RUN_ROOT/logs/${model}_5seed_%j.err" \
    --export="ALL,MODEL=$model,PROJECT_DIR=$PROJECT_DIR,RUN_ROOT=$RUN_ROOT,CACHE_ROOT=$RUN_ROOT/cache" \
